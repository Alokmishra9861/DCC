...
const OrgSignupForm = ({ role }) => {
  ...
  const validate = () => {
    ...
    // ✅ FIX: validate categoryId (not categoryName) so selecting shows as valid
  ...
  useEffect(() => {
    ...
    categoryAPI
      ...
      .then((data) => {
        if (!isMounted) return;
        // ✅ FIX: api.js request() already unwraps { success, data } → returns data array directly
        // but guard all shapes just in case
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      >
        ...
        {/* ✅ FIX: Proper <select> for category — no free-text, always captures ID */}
        {roleUpper === "BUSINESS" && (
          <div>
            ...
            {categoriesLoading ? (
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  field("categoryId", e.target.value); // use name as id fallback
                }}
                className={getInputCls(false)}
